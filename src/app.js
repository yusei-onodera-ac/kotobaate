...
function newQuestion() {
  ...
  $('#answerInput').value = '';
  $('#answerInput').maxLength = getWordLength(selection.word);
  $('#answerInput').ariaLabel = `${getWordLength(selection.word)}文字のひらがなを入力`;
  $('#answerInput').focus();
...
function renderGame() {
  ...
  $('#categoryHint').textContent = `カテゴリ：${state.current.category} / ${getWordLength(state.current.word)}文字（ひらがなのみ）`;
...
function submitGuess(event) {
  ...
  const validation = validateGuess(state.current.word, $('#answerInput').value);
  if (!validation.isValid) {
    $('#message').textContent = validation.message;
    return;
  }

  const guess = validation.normalizedGuess;
...
function tickTimer() {
  ...
  if (state.remaining <= 0) {
    ...
    $('#resultText').textContent = `${MODES[state.mode].timeLimit}秒で${state.solved}問正解、最大${state.streak}コンボ。ランキングを確認しよう。`;
    $('#resultCard').classList.remove('hidden');
  }
}
...
function enforceHiraganaInput(event) {
  const sanitized = [...event.target.value].filter((char) => isHiraganaWord(char)).join('');
  if (event.target.value !== sanitized) {
    event.target.value = sanitized;
    $('#message').textContent = '入力できるのは、ひらがなのみです。';
  }
}
...
function bindEvents() {
  ...
  $('#answerInput').addEventListener('input', enforceHiraganaInput);
}
